...
void TabWidget::addWebAction(QAction *action, QUltralightPage::WebAction webAction)
{
    if (!action)
        return;
...
WebView *TabWidget::newTab(bool makeCurrent)
{
    ...
    connect(webView->page(), SIGNAL(printRequested(QUltralightFrame*)),
            this, SIGNAL(printRequested(QUltralightFrame*)));
...
WebActionMapper::WebActionMapper(QAction *root, QUltralightPage::WebAction webAction, QObject *parent)
    : QObject(parent)
    , m_currentParent(0)
    , m_root(root)
...
QUltralightPage::WebAction WebActionMapper::webAction() const
{
    return m_webAction;
}
...
void WebActionMapper::updateCurrent(QUltralightPage *currentParent)
{
    if (m_currentParent)
