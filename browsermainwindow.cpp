    ...
    , m_reload(0)
{
    ...
    connect(m_tabWidget, SIGNAL(printRequested(QUltralightFrame*)),
            this, SLOT(printRequested(QUltralightFrame*)));
...
void BrowserMainWindow::setupMenu()
{
    ...
    m_tabWidget->addWebAction(m_stop, QUltralightPage::Stop);

    m_reload = viewMenu->addAction(tr("Reload Page"));
    m_reload->setShortcuts(QKeySequence::Refresh);
    m_tabWidget->addWebAction(m_reload, QUltralightPage::Reload);
    ...
    m_tabWidget->addWebAction(m_historyBack, QUltralightPage::Back);
    ...
    m_tabWidget->addWebAction(m_historyForward, QUltralightPage::Forward);
...
void BrowserMainWindow::printRequested(QUltralightFrame *frame)
{
...
{
    QUltralightSettings *settings = QUltralightSettings::globalSettings();
    bool pb = settings->testAttribute(QUltralightSettings::PrivateBrowsingEnabled);
    if (!pb) {
        ...
        if (button == QMessageBox::Ok) {
            settings->setAttribute(QUltralightSettings::PrivateBrowsingEnabled, true);
        }
    } else {
        settings->setAttribute(QUltralightSettings::PrivateBrowsingEnabled, false);
...
void BrowserMainWindow::slotEditFindPrevious()
{
    if (!currentTab() && !m_lastSearch.isEmpty())
        return;
    currentTab()->findText(m_lastSearch, QUltralightPage::FindBackward);
}
...
void BrowserMainWindow::slotViewZoomTextOnly(bool enable)
{
    if (!currentTab())
        return;
    currentTab()->page()->settings()->setAttribute(QUltralightSettings::ZoomTextOnly, enable);
}
...
{
    QUltralightSettings::globalSettings()->setAttribute(QUltralightSettings::DeveloperExtrasEnabled, enable);
    if (enable) {
...
void BrowserMainWindow::slotAboutToShowBackMenu()
{
    ...
    if (!currentTab())
        return;
    QUltralightHistory *history = currentTab()->history();
    int historyCount = history->count();
    for (int i = history->backItems(historyCount).count() - 1; i >= 0; --i) {
        QUltralightHistoryItem item = history->backItems(history->count()).at(i);
...
void BrowserMainWindow::slotAboutToShowForwardMenu()
{
    ...
    for (int i = 0; i < history->forwardItems(history->count()).count(); ++i) {
        QUltralightHistoryItem item = history->forwardItems(historyCount).at(i);
        QAction *action = new QAction(this);
